...
class Trajectory:
    ...
    def store_transition(self, info):
        ...
        self.ep['random_feat'].append(copy.deepcopy(info['random_feat']))
        self.length += 1
    ...
    def sample(self):
        ...
        info = {
            ...
            'random_feat': [copy.deepcopy(self.ep['random_feat'][idx])],
            'done_r': [self.sum_rews]
        }
        return info
...
class ReplayBuffer_RRD:
    ...
    def store_transition(self, info):
        ...
        if self.length>self.buffer_size:
            ...
            self.tgt_random_feat = self.tgt_random_feat[del_len:]
        self.tgt_random_feat += [info['random_feat'], ]
        self.step_counter += 1
        self.in_head = info['real_done']
        if info['real_done']:
            self.ep_counter += 1
            self.r_ep += [self.ep[-1].sum_rews]
    ...
    def sample_batch(self, batch_size=-1, rrd_batch_size=-1, rrd_sample_size=-1):
        ...
        batch = dict(obs=[], obs_next=[], acts=[], rews=[], done=[], done_r=[], random_feat=[], rrd_obs=[], rrd_obs_next=[], rrd_acts=[], rrd_rews=[])
    ...
    def get_all_random_feat(self):
        tgt_random_feat = np.concatenate(self.tgt_random_feat, axis=0, dtype=np.float32).reshape(len(self.tgt_random_feat), -1)
        return tgt_random_feat
    
    def get_norm(self, r):
        r_ep = np.array(self.r_ep)
        r = np.array(r)
        if r_ep.max() -  r_ep.min() > 0:
            norm_r = (r - r_ep.min()) / (r_ep.max() -  r_ep.min())
        else:
            norm_r = 0
        return norm_r
